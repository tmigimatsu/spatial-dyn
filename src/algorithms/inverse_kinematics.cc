...
const double kTolerancePos = 1e-6;
const double kToleranceOri = 1e-6;
...
Eigen::VectorXd InverseKinematics(const ArticulatedBody& ab_in,
  ...
  for (size_t i = 0; i < kMaxIterations; i++) {
    ...
    Eigen::VectorXd ddq = ctrl_utils::PdControl(ab.q(), q_des, ab.dq(), kKpKvJoint);
    tau_cmd += spatial_dyn::opspace::InverseDynamics(ab, I, ddq, &N);

    // Integrate
    spatial_dyn::Integrate(ab, tau_cmd, 1. / kTimerFreq, {}, IntegrationOptions(false));
